...
const resolved = computed(() => {
  ...
  if (raw && typeof raw === "object") return raw as { patch?: string; path?: string };
  try {
    const obj = JSON.parse(String(props.input)) as { patch?: string; path?: string };
    if (obj && typeof obj === "object") return obj;
  } catch { /* fallthrough */ }
  return { patch: String(props.input) };
});
const patch = computed(() => parsePatch(resolved.value.patch ?? ""));
...
<template>
  <div v-if="patch" class="e-lines">
    ...
    <div v-for="(line, i) in patch.lines" :key="i" class="e-line" :class="`e-${line.op}`">
      <span v-if="['replace', 'insert', 'delete', 'append', 'hunk'].includes(line.op)" class="e-hunk" :class="`e-hunk-${line.op}`">@@ {{ line.op }} {{ line.range }} @@</span>
