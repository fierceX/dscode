...
| 信念度区间 | 控制动作 | 类比 |
|:----------:|---------|------|
| ≥ 0.70 | 无操作，自由运行 | 稳态，无需干预 |
| 0.50~0.70 | 温和提醒（Reminder） | 预警区，轻微校正 |
| 0.30~0.50 | 严重警告（Warning） | 保护区，强力校正 |
| < 0.30 | 紧急中止（Abort） | 极限区，切断动力 |
...
- **其他工具**（Read/Write/Edit/Glob/Grep/WebSearch/WebFetch）：Rust 函数直接执行，无退出码。失败时 `Result::Err` 格式化为 `"Error: ..."` 前缀，`collect()` 通过 `full_content.starts_with("Error:")` 捕获。
...
4. 如果含 ToolFailed/ToolError，将错误详情记入 `recent_errors`

新用户输入触发 `reset()`，清空窗口，信念回到 0.75。
...
```
B < 0.30 ─→ Abort（中断本轮，返回 Failed）
0.30 ≤ B < 0.50 → Inject(Warning)
0.50 ≤ B < 0.70 → Inject(Reminder)
B ≥ 0.70 ─→ None（不干预）
```
...
注入发生在 `turn.rs::execute()` 的循环内部，工具执行完成后、下一轮 LLM 调用之前。`TurnExecutor` 持有一个持久化的 `DecisionEngine` 实例，每次 Phase 4 调用它的 `decide()`：

```
Phase 3: 工具执行 → SignalCollector.collect() → belief.observe()
Phase 4: stop = "tool_use"
  ├─ engine.decide(belief, errors)
  │   ├─ 引擎内部检查冷却 → 跳过
  │   ├─ Inject → store.add_user(msg) + 引擎内部激活冷却 + 恢复首步守卫
  │   └─ Abort  → return Failed（绕过冷却）
  └─ messages = compaction.active_messages() → 下一轮 LLM
```

注入消息是一条独立的 User 消息，写入对话存储（`store.add_user()`）。不修改 system prompt（保护前缀缓存），也不追加到用户输入末尾。LLM 在下一轮调用时自然看到。

### 5.3 注入内容

```
[System note: belief 0.37 indicates repeated tool failure. Enter SIGNAL_RECOVERY mode. Your next tool call must be a current-state inspection using one of these active providers: <resolved providers>. Calls that do not satisfy an approved inspection scope will be blocked.
Recent reliability signals:
- Bash(cargo build): Rust compilation error (error[E0308])
- Bash(cargo build): process exited with code 1]
```

`DecisionEngine` 只产生结构化 `RecoveryDirective`；`RecoveryPolicy` 根据当前
`ResolvedToolCapabilities` 选择实际可用的 primary providers 并渲染消息，不在静态 prompt
或决策引擎中硬编码工具组合。没有任何 inspection provider 时，消息要求模型报告限制并停止，
Recovery 首步守卫继续阻止全部工具调用。
...
| 级别 | B 范围 | 错误条数 | 格式 |
|------|:------:|:--------:|------|
| Reminder | 0.50~0.70 | 最近 3 条 | `[System note: belief X.XX is below the recovery threshold...]` |
| Warning | 0.30~0.50 | 最近 5 条 | `[System note: belief X.XX indicates repeated tool failure...]` |
...
统一使用英文，不做本地化。设置 `MINK_SIGNAL_MODE=off` 时，不生成 `<belief-awareness>` 区块，也不执行信号采集、信念更新、注入、Abort 和恢复首步守卫。
...
│   └─ recent_errors.push(detail)                             │
│                                                             │
│ 标题栏实时更新 belief()                                      │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌─ Phase 4: stop="tool_use" ──▼──────────────────────────────┐
│ TurnExecutor 持有持久化 DecisionEngine 实例                  │
│ engine.decide(belief, &recent_errors)                       │
│   ├─ 引擎内部检查冷却 → 跳过                                │
...
│   [User] [System note: belief ... Enter SIGNAL_RECOVERY...] │
│   → LLM 先读取/搜索/验证，再修改                              │
└────────────────────────────────────────────────────────────┘
```
...
`recent_errors` 为空时，注入内容省略 `Recent reliability signals:` 段落，只保留 `[System note: belief X.XX ... Enter SIGNAL_RECOVERY mode ...]`。

### 7.5 冷却期内信念继续恶化

如果冷却期内信念继续恶化至 B < 0.30，Abort 绕过冷却直接中止，安全优先。
...
| BeliefTracker | `agent/belief.rs` | `new()`, `observe()`, `belief()`, `reset()` |
| DecisionEngine | `agent/decision.rs` | `new()`, `decide()`, `reset()`, `cooldown_remaining()` |
...
| 文件 | 职责 |
|------|------|
| `crates/mink-core/src/agent/decision.rs` | 结构化恢复决策、阈值、冷却和 reset |
| `crates/mink-core/src/agent/recovery_policy.rs` | 从已解析能力渲染恢复消息并校验恢复首个调用 |
| `crates/mink-core/src/agent/turn.rs` | 注入恢复消息并启用恢复首步守卫 |
| `crates/mink-core/src/config.rs` | 定义 `SignalMode`，读取 `MINK_SIGNAL_MODE`，控制信号系统开关 |
| `crates/mink-core/src/prompt/core.rs` | `MINK_SIGNAL_MODE=full` 时生成不含具体工具名的 `<belief-awareness>` core |
...
- **冷却参数可配置化**：当前硬编码，后续可在 `.minkrc` 或 `Config` 中配置
