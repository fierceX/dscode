...
为了让模型在被注入时理解上下文并主动调整行为，系统提示词中加入 `<belief-awareness>` 区块，位于 `<verification-gate>` 之后、`<stop-triggers>` 之前：
...
`recent_errors` 为空时，注入内容省略 `Recent errors:` 段落。只显示 `[System note: Some tool executions showed issues (belief X.XX).]`。
...
| `src/agent/turn.rs` | 移除 `cooldown_remaining`，持 `decision_engine` 字段 |
| `src/prompt.rs` | 新增 `<belief-awareness>` 区块 |
