...
SignalCollector.collect(name, output, exit_code, content)
       ...
       │
       ▼
BeliefTracker.observe(signals)     # 先验/窗口/衰减来自 Config.signal
...
响应层级由 `MINK_SIGNAL_TIERS` 门控（evidence / state_ops / restart / full），
允许按部署环境裁剪状态操作、策略重启与接管的行为面；全部阈值与超参位于 `Config.signal`。
...
统一使用英文，不做本地化。设置 `MINK_SIGNAL_MODE=off` 时，不生成 `<belief-awareness>`
...
组件零外部依赖；阈值/先验/窗口/衰减全部来自 `Config.signal`。
...
| `crates/mink-core/src/config.rs` | `SignalMode`/`SignalResponseTiers`/`SignalConfig`，读取 `MINK_SIGNAL_MODE`/`MINK_SIGNAL_TIERS` |
