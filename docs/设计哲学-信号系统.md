...
| 文件 | 职责 |
|------|------|
| `src/agent/decision.rs` | 决策阈值、冷却、reset、cooldown 查询 |
| `src/agent/turn.rs` | 持有 `decision_engine` 字段；注入后启用恢复首步守卫 |
| `src/agent/signal_mode.rs` | 读取 `DSCODE_SIGNAL_MODE`，控制信号系统开关 |
| `src/prompt.rs` | `DSCODE_SIGNAL_MODE=full` 时生成 `<belief-awareness>` 区块 |
