...
信号系统是 dscode 的反馈回路核心。每次工具调用后采集执行质量的信号，合并为单一信念度，根据信念度自动决定是否向 LLM 注入修正提示或中止任务。整个回路由三个组件串联：
...
为了让模型在被注入时理解上下文并主动调整行为，默认系统提示词中加入 `<belief-awareness>` 区块，位于 `<verification-gate>` 之后、`<stop-triggers>` 之前：
...
统一使用英文，不做本地化。设置 `DSCODE_SIGNAL_MODE=off` 时，不生成 `<belief-awareness>` 区块，也不执行信号采集、信念更新、注入、Abort 和恢复首步守卫。
...
### 8.2 DecisionEngine 接口变化

```rust
// 旧
pub fn decide(&self, belief: f64, errors: &[String]) -> Decision

// 新——改为 &mut self，冷却由引擎内部管理
pub fn decide(&mut self, belief: f64, errors: &[String]) -> Decision
pub fn reset(&mut self)
pub fn cooldown_remaining(&self) -> usize
```

`TurnExecutor` 不再维护冷却计数器，改为持有一个持久化的 `DecisionEngine`：
...
| 文件 | 变更 |
|------|------|
| `src/agent/decision.rs` | `decide()` 改 `&mut self`，内部管理冷却；新增 `reset()`、`cooldown_remaining()` |
| `src/agent/turn.rs` | 移除 `cooldown_remaining`，持 `decision_engine` 字段；注入后启用恢复首步守卫 |
| `src/agent/signal_mode.rs` | 读取 `DSCODE_SIGNAL_MODE`，控制信号系统开关 |
| `src/prompt.rs` | 默认新增 `<belief-awareness>` 区块；`DSCODE_SIGNAL_MODE=off` 时省略 |
| `src/test_mock.rs` | 更新 `decide()` 调用处，去掉第 3 参数 |
