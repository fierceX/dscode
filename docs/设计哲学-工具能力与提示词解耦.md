...
- TodoWrite 未进入当前 surface，prompt 仍把它写成必选步骤；
- 子代理仍被提示调用 SubAgent；
...
| 控制流程 | `TodoState`、`PlanDraft`、`PlanConfirm`、`PlanClear`、`Delegation` |
...
| `plan-lifecycle` | `PlanDraft + PlanConfirm + PlanClear` | 绑定草稿、确认和清理 provider；TodoState 存在时追加执行跟踪提示 |
